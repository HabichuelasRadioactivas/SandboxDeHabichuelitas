...
from load_assets import KING_SPRITE
...
class Game(arcade.View):
    def __init__(self):
        ...
        self.talked_to_king_pre_fight = False
        self.talked_to_king_after_fight = False
        self.beat_final_knight = False
        self.first_power_up = False
        self.second_power_up = False
    ...
    def setup(self):
        ...
        self.king.scale = 0.1
        self.king.center_x = SCREEN_WIDTH - 300
        self.king.center_y = SCREEN_HEIGHT / 2 + 30

        self.player_list.append(self.player_sprite)

        self.enemy_list = arcade.SpriteList()
        self.final_knight = FinalKnight()
        self.final_knight.center_x = SCREEN_WIDTH - 50
        self.final_knight.center_y = 340
    ...
    def first_map_logic(self):
        ...
        if self.player_at_first_map_exit() and self.mr_bean_sprite.talked_to_bean is True:
        ...
        if self.mr_bean_sprite.is_near(self.player_sprite.center_x, self.player_sprite.center_y):
            if self.mr_bean_sprite.talked_to_bean is False:
                ...
                if self.first_power_up is not True:
                    self.power_up_first()
                    self.first_power_up = True
    ...
    def third_map_logic(self):
        ...
        if len(self.enemy_list) == 0:
            ...
            if self.second_power_up is not True:
                self.power_up_second()
                self.second_power_up = True
    ...
    def fourth_map_logic(self):
        if self.talked_to_king_pre_fight is False:
            self.talked_to_king_pre_fight = True
            self.window.open_king_cutscene()
            self.npc_list.append(self.king)
        self.start_engine()
        if len(self.enemy_list) == 0:
            if self.beat_final_knight is not True:
                self.enemy_list.append(self.final_knight)
                self.beat_final_knight = True
            elif self.talked_to_king_after_fight is False:
                self.talked_to_king_after_fight = True
                self.npc_list.remove(self.king)  # remove mr_bean from npc list if first map_pngs is left
                self.window.open_post_fight_cutscene()
    ...
    def power_up_first(self):
        self.sound_player.play_sound(sound_name="power_up_sound")
        self.sound_player.reload_sound(sound_name="power_up_sound")
        self.player_sprite.scale = SPRITE_SCALING + 0.2
